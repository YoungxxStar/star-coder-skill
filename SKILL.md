...
4. Summarize the resolved non-secret configuration. Warn before binding to a public interface, disabling authentication, writing a system-wide service, or running on an HPC login node.
...
6. Verify the generated status command and HTTP health endpoint. Report the access URL or SSH tunnel, password retrieval location, control commands, log path, and whether startup survives reboot/container recreation.
...
5. How will it be accessed: local browser, SSH tunnel, container host-port forwarding, trusted LAN, or public reverse proxy? Resolve this to a bind address; default to `127.0.0.1` for SSH/HPC and `0.0.0.0` only for intentional container/LAN forwarding.
